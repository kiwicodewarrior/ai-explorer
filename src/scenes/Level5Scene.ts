...
const REST_OPTIONS = [
  {
    ...
    summary: "Recover before the next portal.",
    detail: "Refill your health for the next stage.",
    fillColor: 0x1f4d53,
...
export class Level5Scene extends Phaser.Scene {
  ...
  create(data: Level5SceneData = {}) {
    ...
    this.currentHealth = Phaser.Math.Clamp(
      data.currentHealth ?? this.selectedCharacter.maxHealth,
      1,
  ...
  private createCharacterPreview() {
    ...
    this.add
      .text(
        GAME_WIDTH / 2 - 56,
        154,
        `${this.selectedCharacter.role} | Speed ${this.selectedCharacter.speed} | Health ${this.currentHealth}/${this.selectedCharacter.maxHealth}`,
        {
          fontFamily: "system-ui, sans-serif",
          fontSize: "16px",
          color: "#bad3ea",
      ...
      if (confirmPressed) {
        if (this.selectedReward === "skip") {
          this.scene.start("level-7", {
            ...
            startingHealth: this.currentHealth,
          });
        } else {
          this.scene.start("level-6", {
            characterId: this.selectedCharacter.id,
            startingHealth: this.selectedCharacter.maxHealth,
          });
        }
      }
      return;
  ...
  private updateSelectionVisuals() {
    this.optionCards.forEach((card, index) => {
      ...
      card.badge.setText(card.key === "restore" ? `Restore to ${this.selectedCharacter.maxHealth}` : "Skip Level 6");
  ...
  private confirmChoice() {
    ...
    if (selectedOption.key === "restore") {
      this.registry.set("level5HealthRestore", this.selectedCharacter.maxHealth);
      this.statusText.setText(
        `Health restored from ${this.currentHealth} to ${this.selectedCharacter.maxHealth}. Press Enter or Space for Level 6.`,
      );
    } else {
    ...
    this.footerText.setText("Level 5 complete. Level 8 is not built yet, so the current path ends after Level 7.");
